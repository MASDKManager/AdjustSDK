package com.fis.fisdk.ui.nativeui;
...
import com.fis.fisdk.models.api.model.response.AskCodeAgain;
import com.fis.fisdk.models.api.model.response.BackGround;
import com.fis.fisdk.models.api.model.response.Disclaimers;
import com.fis.fisdk.models.api.model.response.Layout;
import com.fis.fisdk.models.api.model.response.OTPHeaderIcon;
import com.fis.fisdk.models.api.model.response.Theme;
...
public class Action3Activity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        close.setOnClickListener(view -> {
            ...
            Utils.logEvent(getBaseContext(), Constants.pin_verify_close, "");
            finish();
        });


        Utils.logEvent(getBaseContext(), Constants.pin_verify_opened, "");
                            ...
                            initiateService.initiate(RetrofitClient.BASE_URL, RetrofitClient.header, encryptedBody).enqueue(new Callback<String>() {
                                ...
                                public void onResponse(Call<String> call, Response<String> response) {
                                    ...
                                    if (response.isSuccessful()) {
                                        Utils.logEvent( getBaseContext(), Constants.pin_verify_api_ok, "");
                                        ...
                                        if (apiResponse.getError() == 0 && apiResponse.getNextAction() != null && apiResponse.getNextAction().getAction() == 7) {
                                            Utils.logEvent(getBaseContext(), Constants.pin_verify_ok, "");
                                            Utils.logEvent(getBaseContext(), Constants.pin_verify_sub_attempt, "");
                                            //
                                            finish();
                                        } else if (apiResponse.getError() > 0) {
                                            Utils.logEvent(getBaseContext(), Constants.pin_verify_error, "");
                                            error_field.setVisibility(View.VISIBLE);
                                            error_field.setText(apiResponse.getMessageToShow());
                                        }
                                    }else{
                                        Utils.logEvent(getBaseContext(), Constants.pin_verify_api_unsuccessful, "");
                                    }
                                }
                                ...
                                public void onFailure(Call<String> call, Throwable t) {
                                    hideLoader();
                                    Utils.logEvent(getBaseContext(), Constants.pin_verify_api_error, "");
    ...
    private void wrongNumber() {
        ...
        intent.putExtra("layout", MobFlow.SendPinLayout);
        intent.putExtra("id", MobFlow.SendPinSessionId);
        int action = getIntent().getIntExtra("action", 3);
        action = action - 1;
        ...
        initiateService.initiate(RetrofitClient.BASE_URL, RetrofitClient.header, encryptedBody).enqueue(new Callback<String>() {
            ...
            public void onResponse(Call<String> call, Response<String> response) {
                ...
                if (response.isSuccessful()) {
                    ...
                    Utils.logEvent(getBaseContext(), Constants.pin_verify_retry_ok, "");
                    String res = Utils.decrypt(response.body(), RetrofitClient.encKey);
                    Gson gson = new Gson();
                    ApiResponse apiResponse = gson.fromJson(res, ApiResponse.class);
                    ...
                    if (apiResponse.getNextAction() != null && apiResponse.getNextAction().getAction() == 7) {
                        Utils.logEvent(getBaseContext(), Constants.pin_verify_retry_error, "");
                        finish();
                    }
                }
            }
    ...
    private void defineResendPin(AskCodeAgain askCodeAgain) {
        if (askCodeAgain != null) {
            ...
            long min = ((askCodeAgain.getCountDown() * 1000L) / 60000) % 60;
            long sec = ((askCodeAgain.getCountDown() * 1000L) / 1000) % 60;
